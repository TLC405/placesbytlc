...
import { ArrowLeft, Calendar, Heart, TrendingUp, Droplets, Sun } from "lucide-react";
...
export default function PeriodTracker() {
  ...
  const getNextPeriod = () => {
    if (!data.lastPeriodStart) return null;
    return new Date(new Date(data.lastPeriodStart).getTime() + data.cycleLength * 24 * 60 * 60 * 1000);
  };
  ...
  const getFertileWindow = () => {
    ...
    return {
      start: new Date(ovulation.getTime() - 5 * 24 * 60 * 60 * 1000),
      end: new Date(ovulation.getTime() + 1 * 24 * 60 * 60 * 1000),
      ovulation,
    };
  };
  ...
  const getCurrentPhase = () => {
    ...
    if (daysSince < data.periodLength) return { name: "Menstrual", emoji: "🌙", color: "text-rose-500", desc: "Period days - rest and self-care" };
    if (daysSince < data.cycleLength - 14 - 5) return { name: "Follicular", emoji: "🌱", color: "text-emerald-500", desc: "Energy rising - great for new activities" };
    if (daysSince < data.cycleLength - 14 + 1) return { name: "Ovulation", emoji: "✨", color: "text-amber-500", desc: "Peak energy - social and creative time" };
  ...
  const logPeriod = () => saveData({ ...data, lastPeriodStart: new Date().toISOString().split("T")[0] });
      ...
      <div className="page-content space-y-6">
          ...
          <section className="animate-in-delay-1">
            <div className="grid grid-cols-2 gap-3">
              <div className="card-premium text-center py-4">
                ...
                <p className="text-lg font-bold text-foreground">{nextPeriod ? formatDate(nextPeriod) : "—"}</p>
        ...
        {fertileWindow && (
          <section className="animate-in-delay-2">
            <div className="section-header"><h2 className="section-title">Fertile Window</h2></div>
            <div className="card-premium">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-medium text-foreground">{formatDate(fertileWindow.start)} – {formatDate(fertileWindow.end)}</p>
                  <p className="text-xs text-muted-foreground">Ovulation around {formatDate(fertileWindow.ovulation)}</p>
                </div>
              </div>
            </div>
          </section>
        ...
        {currentPhase && (
          <section className="animate-in-delay-2">
            <div className="section-header"><h2 className="section-title">Date Ideas for This Phase</h2></div>
          ...
          <div className="card-premium space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => saveData({ ...data, cycleLength: Math.max(21, data.cycleLength - 1) })} className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-foreground">−</button>
                <span className="w-10 text-center font-semibold text-foreground">{data.cycleLength}</span>
                <button onClick={() => saveData({ ...data, cycleLength: Math.min(35, data.cycleLength + 1) })} className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-foreground">+</button>
              </div>
            </div>
            <div className="border-t border-border pt-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => saveData({ ...data, periodLength: Math.max(3, data.periodLength - 1) })} className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-foreground">−</button>
                <span className="w-10 text-center font-semibold text-foreground">{data.periodLength}</span>
                <button onClick={() => saveData({ ...data, periodLength: Math.min(8, data.periodLength + 1) })} className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-foreground">+</button>
              </div>
            </div>
            {data.lastPeriodStart && (
              <div className="border-t border-border pt-4">
                <button onClick={logPeriod} className="btn-secondary w-full">
                  <Droplets className="w-4 h-4" />
